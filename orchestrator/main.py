...
def post_recovery(relinquished_ids, deactivated_ids, unchanged_deactivated_ids, src_server_id, dest_server_id):
    ...
    unchanged_remaining_ids = list_difference(unchanged_deactivated_ids, relinquished_ids)
...
def transfer_inventory(inventory_ids, current_location, new_location):
    if current_location != 0:
        ...
        if curr_partner_id:
            deactivated_ids_partner = request_deactivation(curr_partner_id, inventory_ids, False, new_location)
        
        # Then send (writing) deactivation request to main node
        deactivated_ids_primary = request_deactivation(current_location, inventory_ids, True, new_location)
